<template>
  <div class="page">
    <header class="header">编辑地址</header>
    <main class="main">
      <section class="card">
        <label class="field">
          <span>联系人</span>
...
export default {
  ...
  data() {
    return {
      ...
      addressId: Number(this.$route.query.addressId),
      form: {
        id: Number(this.$route.query.addressId),
        contactName: '',
        contactSex: 0,
        contactTel: '',
        address: ''
  ...
  async mounted() {
    try {
      const response = await this.$axios.get(`/api/addresses/${this.addressId}`)
  ...
  methods: {
    async submit() {
      try {
        const response = await this.$axios.patch('/api/addresses', this.form)
