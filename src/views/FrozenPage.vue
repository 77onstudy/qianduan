          ...
          <div v-else class="freeze-list">
            ...
            >
              <div class="freeze-main">
                <div class="title">
                  {{
                    item.order && item.order.business
                      ? item.order.business.businessName
                      : '订单商家'
                  }}
                </div>
                <div class="amount">
              ...
              <div class="freeze-sub">
                <div class="row">
                  ...
                  <span class="value">
                    ￥{{
                      item.order && item.order.orderTotal != null
                        ? Number(item.order.orderTotal).toFixed(2)
                        : '0.00'
                    }}
                  </span>
                </div>
                ...
                <div class="row">
                  ...
                  <span class="value">{{ formatStatus(item) }}</span>
                </div>
...
export default {
  ...
  methods: {
    ...
    // 现在后端返回：
    // { success: true, code: 'OK', data: [ { ... 冻结记录 ... } ] }
    loadList() {
      return this.$axios
        .get('/api/wallet/listFreeze')
        .then(res => {
    ...
    formatTime(t) {
      if (!t) return '-';
      // 兼容 "2025-11-14T08:45:49.146678" 这种格式
    ...
    // 状态文字：优先用 frozen / unfrozen 布尔字段
    formatStatus(item) {
      if (!item) return '未知';
      if (item.frozen) return '冻结中';
      if (item.unfrozen) return '已解冻';
